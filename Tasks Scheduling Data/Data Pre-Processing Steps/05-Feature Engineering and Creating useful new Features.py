...
df['cpu_usage_distribution_modified_listed'] = df['cpu_usage_distribution_modified'].apply(lambda x: ast.literal_eval(x) if not pd.isna(x) else np.nan)



# Calculate the mean of each list and store the results in a new column
df['mean_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(calculate_mean)

# Find the maximum value of each list and store the results in a new column
df['max_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(find_max)

# Find the minimum value of each list and store the results in a new column
df['min_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(find_min)

# Find the variance value of each list and store the results in a new column
df['variance_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(find_variance)


# Find the mode value of each list and store the results in a new column
df['mode_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(find_mode)


# Find the median value of each list and store the results in a new column
df['median_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(find_median)


# Find the difference between the maximum and minimum values of each list and store the results in a new column
df['max_min_diff_cpu_usage'] = df['cpu_usage_distribution_modified_listed'].apply(find_max_min_diff)
